...
module.exports = {
    ...
    run: async (client, message, args, { GuildDB }) => {
        ...
        if (!SearchString) return client.sendTime(message.channel, `**กรุณาเข้า - **\`${GuildDB.prefix}play [song]\``);
        let CheckNode = client.Manager.nodes.get(client.config.Lavalink.id);
        let Searching = await message.channel.send(":mag_right: กำลังมองหาเพลง...");
        ...
        try {
            if (SearchString.match(client.Lavasfy.spotifyPattern)) {
                ...
                if (Searched.loadType === "PLAYLIST_LOADED") {
                    ...
                    SongAddedEmbed.setAuthor(`เพิ่มพื้นที่เพลง`, client.config.IconURL);
                ...
                if (Searched.loadType === "NO_MATCHES") return client.sendTime(message.channel, "**ไม่พบเพลง - **" + SearchString);
                    ...
                    SongAddedEmbed.addField("ศิลปิน", Searched.tracks[0].author, true);
                    SongAddedEmbed.addField("เวลา", `\`${prettyMilliseconds(Searched.tracks[0].duration, { colonNotation: true })}\``, true);
                    if (player.queue.totalSize > 1) SongAddedEmbed.addField("Position in queue", `${player.queue.size - 0}`, true);
                    Searching.edit(SongAddedEmbed);
                }
            }
        ...
        options: [
            {
                ...
                type: 3,
                required: true,
                description: "เล่นเพลงในห้องของคุณ",
            },
        ],
        /**
         *
            ...
            if (search.match(client.Lavasfy.spotifyPattern)) {
                ...
                switch (Searched.loadType) {
                    case "LOAD_FAILED":
                        ...
                        return client.sendError(interaction, `❌ | **เกิดข้อผิดพลาดขณะค้นหา**`);

                    case "NO_MATCHES":
                        if (!player.queue.current) player.destroy();
                        return client.sendTime(interaction, "❌ | **ไม่พบผลลัพธ์**");
                    case "TRACK_LOADED":
                ...
                try {
                    ...
                    if (res.loadType === "LOAD_FAILED") {
                        if (!player.queue.current) player.destroy();
                        return client.sendError(interaction, `:x: | **เกิดข้อผิดพลาดขณะค้นหา**`);
                    }
                } catch (err) {
                    return client.sendError(interaction, `*เกิดข้อผิดพลาดขณะค้นหา: ${err.message}`);
                }
                switch (res.loadType) {
                    case "NO_MATCHES":
                        if (!player.queue.current) player.destroy();
